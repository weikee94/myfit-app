...
### Added
- Stick-figure demo animations (inline SVG, follows light/dark) for 40 exercise names that have no free photo, shown in the exercise sheet with a "示意 · 以 YouTube 真人示范为准" label and a cue quoted from the plan notes. Poses are drawn from joint angles (`src/lib/stickFigure.ts`, data in `src/data/stickFigures.ts`); reduced-motion shows two still key poses. Fire Hydrant, Plank Pull Through and Plank Twist are intentionally left as YouTube-only (sideways/rotational movements a 2D figure would misrepresent).
